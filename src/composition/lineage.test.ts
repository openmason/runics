...
describe('lineage', () => {
  ...
  describe('getAncestry', () => {
    it('should return ancestry chain with mapped fields', async () => {
      mockPool.query.mockResolvedValueOnce({
        rows: [
          { id: 'root', slug: 'root-skill', name: 'Root', fork_depth: 0 },
          { id: 'child', slug: 'child-skill', name: 'Child', fork_depth: 1 },
          { id: 'grandchild', slug: 'gc', name: 'GC', fork_depth: 2 },
        ],
      });
      ...
      expect(result).toEqual([
        { id: 'root', slug: 'root-skill', name: 'Root', forkDepth: 0 },
        { id: 'child', slug: 'child-skill', name: 'Child', forkDepth: 1 },
        { id: 'grandchild', slug: 'gc', name: 'GC', forkDepth: 2 },
      ]);
    });

    it('should handle null fork_depth', async () => {
      mockPool.query.mockResolvedValueOnce({
        rows: [{ id: 'a', slug: 's', name: 'N', fork_depth: null }],
      });

      const result = await getAncestry('a', mockPool);
      expect(result[0].forkDepth).toBe(0);
    });
  ...
  describe('getForks', () => {
    ...
    it('should query WHERE fork_of = skillId', async () => {
      mockPool.query.mockResolvedValueOnce({ rows: [] });
      await getForks('parent-id', mockPool);
      expect(mockPool.query.mock.calls[0][0]).toContain('fork_of');
      expect(mockPool.query.mock.calls[0][1]).toEqual(['parent-id']);
    });
  });
